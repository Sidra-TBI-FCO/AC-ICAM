...
for (i in 1:10){
  ...
  #png(paste0("./Figures/TCR/7_Overlapping_sequences_scatterplots/7_Patient_", Patient, "_T_N_Scatterplot.png"), width = 9, height = 6,
      #units = "in", res = 600)
  #plot(plot)
  #dev.off()
  ...
  png(paste0("./Figures/TCR/7_Overlapping_sequences_scatterplots/7_Without_label_Patient_", Patient, "_T_N_Scatterplot.png"), width = 9, height = 6,
      units = "in", res = 600)
  plot(plot2)
  dev.off()
...
dir.create("./Analysis/TCR/7_Overlapping_sequences_analysis", showWarnings = FALSE)
save(results, file = "./Analysis/TCR/7_Overlapping_sequences_analysis/7_Overlapping_sequences_TN_pairs.Rdata")

files = list.files("./Figures/TCR/7_Overlapping_sequences_scatterplots")

i=1
for (i in 1:length(files)){
  file = files[i]
  plot = readPNG(paste0("./Figures/TCR/7_Overlapping_sequences_scatterplots/", file))
  assign(paste0("plot", i), plot)
}

pdf(paste0("Figures/TCR/7_Overlapping_sequences_scatterplots/TN_Scatterplots_combined.pdf"), width = 20, height = 34)
grid.arrange(rasterGrob(plot1),rasterGrob(plot2),rasterGrob(plot3), rasterGrob(plot4), rasterGrob(plot5),
             rasterGrob(plot6),rasterGrob(plot7),rasterGrob(plot8), rasterGrob(plot9), rasterGrob(plot10),
