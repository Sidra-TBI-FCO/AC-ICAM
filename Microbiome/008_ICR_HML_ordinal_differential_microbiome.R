...
for (i in 1:nrow(rank_abundancies)){
  ...
  if(cor$p.value < 0.053 | make_plot == "make_plot"){
    plot = ggplot(df_plot, aes(x = ICR_cluster, y = Abundance)) +
      ...
      stat_summary(fun.y = "median", colour = "yellow", fill = "yellow", size = 2, geom = "point") +
    ...
    png(paste0("./Figures/Microbiome/008_ICR_ordinal_boxplot/", Type, "/", Rank, "/facet_violin_", micr, "_", subset,
               "_by_ICR_cluster_boxplot.png"), res = 600, width = 8, height = 4, units = "in")
    plot(plot)
    dev.off()
  }
