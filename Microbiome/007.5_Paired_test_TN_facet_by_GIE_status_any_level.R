...
IES_df$GIE = IES_df$Immunoedited_ICR_cluster
IES_df$GIE = gsub(" ICR High", "", IES_df$GIE)
IES_df$GIE = gsub(" ICR Medium", "", IES_df$GIE)
IES_df$GIE = gsub(" ICR Low", "", IES_df$GIE)
table(IES_df$GIE)
...
df$GIE = IES_df$GIE[match(df$Patient_ID, IES_df$Patient_ID)]
df$GIE = factor(df$GIE, levels = c("less immunoedited", "immunoedited"))

dir.create("./Figures/Microbiome/007.5_Paired_wilcoxon_test_TN_facet_GIE", showWarnings = FALSE)
dir.create(paste0("./Figures/Microbiome/007.5_Paired_wilcoxon_test_TN_facet_GIE/", Type), showWarnings = FALSE)
dir.create(paste0("./Figures/Microbiome/007.5_Paired_wilcoxon_test_TN_facet_GIE/", Type, "/", Rank), showWarnings = FALSE)
...
for (i in 1:nrow(rank_abundancies)){
  ...
  if(make_plot == "make_plot" | mean > 0.001){
    ...
    png(paste0("./Figures/Microbiome/007.5_Paired_wilcoxon_test_TN_facet_GIE/", Type, "/", Rank, "/v3_",
               micr, "_paired_boxplot_N_T.png"), res = 600, units = "in", width = 4, height = 3.5)
    plot(plot)
    dev.off()
    
    plot2 = ggplot(melt, aes(x = ICR_cluster, y = value)) +
    ...
    #png(paste0("./Figures/Microbiome/007.2_Paired_wilcoxon_test_TN_facet_ICR_cluster/", Type, "/", Rank, "/Facet_TN_x_ICR_",
    #          micr, ".png"), res = 600, units = "in", width = 6, height = 3.5)
    #plot(plot2)
    #dev.off()
  }
}
