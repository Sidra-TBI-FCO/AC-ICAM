...
dir.create("./Figures/Microbiome/034_IES_Spearman_boxplot", showWarnings = FALSE)
dir.create(paste0("./Figures/Microbiome/034_IES_Spearman_boxplot/", Type), showWarnings = FALSE)
dir.create(paste0("./Figures/Microbiome/034_IES_Spearman_boxplot/", Type, "/", Rank), showWarnings = FALSE)
...
df_plot = data.frame(Patient_ID = rownames(abundance_T),
                     ...
                     Mutation_cat = NA,
                     Abundance = NA)
df_plot$IES = IES_df$IES[match(df_plot$Patient_ID, IES_df$Patient_ID)]
df_plot$IES = factor(df_plot$IES, levels = c("IES1", "IES2", "IES3", "IES4"))
table(df_plot$IES)
df_plot$IES_num = as.numeric(df_plot$IES)
table(df_plot$IES_num)
...
df_plot = df_plot[-which(is.na(df_plot$IES)),]
...
for (i in 1:nrow(rank_abundancies)){
  ...
  df_calc = df_plot
  test = cor.test(df_calc$Abundance, df_calc$IES_num, method = "spearman")
  
  results$p_val[which(results$Name == micr)] = test$p.value
  results$correlation_coef[which(results$Name == micr)] = test$estimate
  ...
  if(test$p.value < 0.053 | make_plot == "make_plot"){
    plot = ggplot(df_plot, aes(x = IES, y = Abundance)) +
      stat_summary(fun.y = "median", colour = "grey", size = 1, geom = "point") +
      geom_violin(aes(fill=IES)) +
      geom_boxplot(width=0.1, outlier.shape = NA) +
      scale_fill_manual(values = c("IES1" = "#5233FC", "IES2" = "#009CFC",
                                   "IES3" = "#FF9F00", "IES4" = "#FF3806")) +
    ...
    png(paste0("./Figures/Microbiome/034_IES_Spearman_boxplot/", Type, "/", Rank, "/v3_", micr, "_", subset,
               "_by_ICR_cluster_boxplot.png"), res = 600, width = 4, height = 4, units = "in")
    plot(plot)
    dev.off()
  }
...
dir.create(paste0("./Analysis/Microbiome/034_IES_Spearman"), showWarnings = FALSE)
dir.create(paste0("./Analysis/Microbiome/034_IES_Spearman/",  Type), showWarnings = FALSE)
dir.create(paste0("./Analysis/Microbiome/034_IES_Spearman/",  Type, "/", Rank), showWarnings = FALSE)

save(results, file = paste0("./Analysis/Microbiome/034_IES_Spearman/",  Type, "/", Rank,
                            "/", subset, "_by_ICR_cluster_", Type, "_", Rank, ".Rdata"))
write.csv(results, file = paste0("./Analysis/Microbiome/034_IES_Spearman/",  Type, "/", Rank,
