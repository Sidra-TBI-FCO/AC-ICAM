...
Merged_dataset = Merged_dataset[-which(is.na(Merged_dataset$DSS.Status)),]

# time / event object creation
Y = Surv.cutoff.years * 365
TS.Alive = Merged_dataset[Merged_dataset$DSS.Status == "Alive", c("DSS.Status", "DSS.Time", Group.of.interest, "ICRscore")]
colnames(TS.Alive) = c("Status","Time", Group.of.interest, "ICRscore")
TS.Alive$Time = as.numeric(as.character(TS.Alive$Time))
TS.Alive$Time[TS.Alive$Time > Y] = Y

TS.Dead = Merged_dataset[Merged_dataset$DSS.Status == "Dead", c("DSS.Status", "DSS.Time", Group.of.interest, "ICRscore")]
...
# plots
png(paste0("./Figures/Trimmed_p/Kaplan Meier Plots/Mar_2020_", str_c(exclude, collapse = "_"), "_", exclude_stage_IV, "_", exclude_adjuvant,"_DSS_Survival_", Group.of.interest,"_Surv_cutoff_years_",Surv.cutoff.years,".png"),res=600,height=6,width=8,unit="in")  # set filename
ggkm(mfit,
     timeby=12,
