...
# Script identical to script 003 but using Mann-Whitney U statistic instead of t-test
...
genes = c("TGIF1")
subset = "hypermutated"  # "all_patients" or "nonhypermutated" or "hypermutated"
...
dir.create("./Figures/WES/003.2_Boxplot_ICRscore_MUT_vs_WT", showWarnings = FALSE)
dir.create("./Analysis/WES/003.2_ICRscore_MUT_vs_WT", showWarnings = FALSE)
...
for (j in all_genes){
  ...
  w_test = wilcox.test(plot_df$ICRscore[which(plot_df$Mutation_status == "WT")], plot_df$ICRscore[which(plot_df$Mutation_status == "MUT")],
                       conf.int = TRUE)
  results$mean_WT[which(results$Gene == gene)] = mean(plot_df$ICRscore[which(plot_df$Mutation_status == "WT")])
  results$mean_MUT[which(results$Gene == gene)] = mean(plot_df$ICRscore[which(plot_df$Mutation_status == "MUT")])
  results$p_value[which(results$Gene == gene)] = w_test$p.value
  results$CI_lower[which(results$Gene == gene)] = w_test$conf.int[1]
  results$CI_upper[which(results$Gene == gene)] = w_test$conf.int[2]
...
write.csv(results, file = paste0("./Analysis/WES/003.2_ICRscore_MUT_vs_WT/", subset, "_", selection ,"_table_MUT_WT_ICRscore",
                                 ".csv"),
          row.names = FALSE)
...
write.csv(results_sub_sig, file = paste0("./Analysis/WES/003.2_ICRscore_MUT_vs_WT/", subset, "_", selection ,"_Table_significant_difference_MUT_WT_ICRscore_min_5_MUT.csv"),
          row.names = FALSE)
...
for (i in genes){
  ...
  dir.create(paste0("./Figures/WES/003.2_Boxplot_ICRscore_MUT_vs_WT/", subset), showWarnings = FALSE)
  png(filename = paste0("./Figures/WES/003.2_Boxplot_ICRscore_MUT_vs_WT/", subset, "/Boxplot_Mut_status_", gene, "_vs_ICRscore.png"),
