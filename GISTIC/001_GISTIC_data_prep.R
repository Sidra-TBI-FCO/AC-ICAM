...
amp = read.table("./Processed_Data/WES/GISTIC/amp_genes.conf_95.txt", stringsAsFactors = FALSE,
                 fill = TRUE, sep = "\t")
del = read.table("./Processed_Data/WES/GISTIC/del_genes.conf_95.txt", stringsAsFactors = FALSE,
                 fill = TRUE, sep = "\t")
seg_file = read.table("./Processed_Data/WES/GISTIC/298SampleMerged.seg", stringsAsFactors = FALSE, 
                      sep = "\t", header = TRUE)

df_GISTIC = read.table("./Processed_Data/WES/GISTIC/all_thresholded.by_genes.txt", stringsAsFactors = FALSE,
                                      sep = "\t", header = TRUE)
...
colnames(df_GISTIC)
colnames(df_GISTIC) = gsub("X", "", colnames(df_GISTIC))
colnames(df_GISTIC) = gsub("WES_COAD_LUMC_SIDRA_", "", colnames(df_GISTIC))
colnames(df_GISTIC)[4:ncol(df_GISTIC)] = str_pad(colnames(df_GISTIC)[4:ncol(df_GISTIC)], pad = "0", 4)
...
dir.create("./Analysis/WES/GISTIC", showWarnings = FALSE)
dir.create("./Analysis/WES/GISTIC/001_Data_prep", showWarnings = FALSE)

save(del_genes, amp_genes, file = "./Analysis/WES/GISTIC/001_Data_prep/del_amp_genes_vectors.Rdata")
save(df_GISTIC, file = "./Analysis/WES/GISTIC/001_Data_prep/GISTIC_All_thresholded_by_genes.Rdata")
save(seg_file, file = "./Analysis/WES/GISTIC/001_Data_prep/GISTIC_Seg_file.Rdata")

df_GISTIC = df_GISTIC[,c(1, 2, 3, which(substring(colnames(df_GISTIC), 1, 3) %in% WES_patients))]
gene_data = df_GISTIC[,1:3]
rownames(df_GISTIC) = df_GISTIC$Gene.Symbol
df_GISTIC = df_GISTIC[,-c(1:3)]
...
save(gene_data, df_GISTIC, file = "./Analysis/WES/GISTIC/001_Data_prep/GISTIC_281_patients_thresholded_by_genes.Rdata")
save(seg_file, file = "./Analysis/WES/GISTIC/001_Data_prep/GISTIC_281_patients_Seg_file.Rdata")
write.table(seg_file, file = "./Analysis/WES/GISTIC/001_Data_prep/GISTIC_281_patients_Seg_file.seg", sep = "\t", row.names = FALSE,
            quote = FALSE)

seg_file$Segment_Mean = log(seg_file$Segment_Mean, 2)
write.table(seg_file, file = "./Analysis/WES/GISTIC/001_Data_prep/GISTIC_281_patients_log2_Seg_file.seg", sep = "\t", row.names = FALSE,
            quote = FALSE)
...
write.table(Annotation, file = "./Analysis/WES/GISTIC/001_Data_prep/New_Annotation_for_281_patients.txt", sep = "\t", row.names = FALSE,
            quote = FALSE)
